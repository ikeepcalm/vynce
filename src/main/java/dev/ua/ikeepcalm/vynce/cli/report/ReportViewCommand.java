...
class ReportViewCommand implements Callable<Integer> {
    ...
    private void displaySummaryVulnerabilities(List<Vulnerability> vulnerabilities) {
        ...
        for (Severity severity : Severity.values()) {
            List<Vulnerability> vulnsOfSeverity = vulnerabilities.stream().filter(v -> v.severity() == severity).toList();

            if (!vulnsOfSeverity.isEmpty()) {
                System.out.println("\n" + getSeverityEmoji(severity) + " " + severity.name() + ":");
                for (Vulnerability vuln : vulnsOfSeverity) {
                    System.out.println("  • " + vuln.title());
                    if (vuln.type() != null) {
                        System.out.println("    Type: " + vuln.type().name());
                    }
                }
            }
        }
    ...
    private void displayDetailedVulnerabilities(List<Vulnerability> vulnerabilities) {
        ...
        for (Severity severity : Severity.values()) {
            ...
            if (!vulnsOfSeverity.isEmpty()) {
                ...
                for (Vulnerability vuln : vulnsOfSeverity) {
                    System.out.println("  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
                    System.out.println("  Title: " + vuln.title());
                    if (vuln.type() != null) {
                        System.out.println("  Type: " + vuln.type().name());
                    }
                    System.out.println("  Description: " + vuln.description());
                    System.out.println("  URL: " + vuln.url());
                    if (vuln.payload() != null) {
                        System.out.println("  Payload: " + vuln.payload());
                    }
                }
                System.out.println();
            }
